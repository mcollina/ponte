...
function CoAP(opts, done) {
  ...
  var logger = this._logger = opts.ponte.logger;
...
CoAP.prototype._handleGET = function(topic, req, res) {
  ...
  var cb = function(topic, payload) {
    logger.info({ url: req.url, code: req.code, sender: req.rsinfo }, 'sending update');
    res.write(payload);
  }
  ...
  that._persistence.lookupRetained(topic, function(err, packets) {
    ...
    if (req.headers['Observe'] === 0) {
      logger.info({ url: req.url, code: req.code, sender: req.rsinfo }, 'registering for topic');
    ...
    logger.info({ url: req.url, code: req.code, sender: req.rsinfo }, 'delivering result');
